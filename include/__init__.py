...
class Game:
    ...
    def _get_neighbors_indices(self, y, x):
        ...
        horizontal_idx = [
            loc for loc in (left, center, right) if
            ((loc >= 0) and (loc < self._width))
        ]
        vertical_idx = [
            loc
    ...
    def _get_cell_next_state(self, y, x):
        ...
        if (self._board.get_cell_state(y, x) and (
                alive_neighbor_count in (2, 3))) or (
                not self._board.get_cell_state(y, x) and (
